...
class AppKey extends Component {
  ...
  getTabActiveKey(match){
    ...
    let base = window.routerBase.substring(0, window.routerBase.lastIndexOf('/'));
    let key = location.pathname.replace(`${base + match.path}/`, '');
  ...
  render() {
    ...
    const tabList = (appInfo.appStatus < 5 || appInfo.appStatus == 6) ? tablist : tablist.slice(0, -1);

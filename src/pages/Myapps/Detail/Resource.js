  ...
  render() {
    ...
    const sourceColumns = [
      ...
      },
      {
        title: '下载',
        render: (text, record) => (
          <Fragment>{record.file_url && <a href={record.file_url}>下载</a>}</Fragment>
        ),
      },
    ];
    ...
    const upload_grammar = {
      name: 'file',
      action: '/dev/grammar/ajaxAddGrammar',
      headers: {
      ...
      <GridContent>
        <Card
          title={
            <span>
              ...
              />
              <Tooltip
                title={<FormattedMessage id="help.sdk_res" defaultMessage="resource download" />}
              >
        ...
        >
          <Table
            ...
            onRow={record=>{
              return {
                onClick: ()=>{
                  let expandedRows = this.state.expandedRows;
                  if (expandedRows.indexOf(record.key) > -1) {
                    removeFromArray.call(expandedRows, record.key);
                  } else {
                    expandedRows.push(record.key);
                  }
          ...
          <div style={{ marginTop: 15 }}>
            <Button type="primary" onClick={() => this.allExpand(resourceList)}>
        ...
        <Card
          title={
            <span>
              <FormattedMessage id="myapps.detail.grammar.file" defaultMessage="grammar download" />
              <Tooltip
                title={
        ...
        >
          <Fragment>
            ...
            <div style={{ marginLeft: 100, marginTop: 10 }}>
              仅支持语法文件格式：wordlist和.jsgf
              <Tooltip
                title={
                  <FormattedMessage
                    id="help.grammar.res.ext"
                  />
                }
              >
                <Icon type="question-circle" theme="outlined" style={{ marginLeft: 10 }} />
